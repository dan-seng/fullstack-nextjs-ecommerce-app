  ...
  return (
    <div className="min-h-screen bg-background py-8">
      <div className="container mx-auto px-4 flex flex-col md:flex-row gap-12 items-center max-w-6xl">
        {/* Product Image */}
        <div className="relative h-96 w-full md:w-1/2 rounded-2xl overflow-hidden bg-muted border-border">
          <Image
            alt={product.name}
            src={imageSrc}
            fill
        ...
        <div className="w-full md:w-1/2 space-y-6">
          ...
          {quantity > 0 ? (
            <div className="flex items-center space-x-6">
              <div className="flex items-center space-x-4 bg-muted rounded-xl p-2 border-border">
                <Button
                  variant="ghost"
                  onClick={() => removeItem(product.id)}
                  disabled={quantity === 0}
                  className="h-10 w-10 p-0 hover:bg-accent disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200 rounded-lg"
                >
                  -
                </Button>

                <span className="text-lg font-semibold text-foreground min-w-8 text-center">
                  {quantity}
                </span>
                ...
                <Button
                  variant="ghost"
                  onClick={onAddItem}
                  className="h-10 w-10 p-0 hover:bg-accent transition-all duration-200 rounded-lg"
                >
                  +
                </Button>
              </div>
